...
SCOUT_SCRIPT = Path(r"C:\Users\Matthew\Documents\claude\smart-scout\smart_scout.py")
...
def check_and_restart():
    if HEARTBEAT_FILE.exists():
        try:
            ...
            if age < MAX_STALE_SECONDS:
                print(f"[OK] Scout alive (PID {old_pid}, last beat {age:.0f}s ago, "
                      f"{data.get('send_count', 0)} sends, state={data.get('state', '?')})")
                return
            print(f"[!] Heartbeat stale ({age:.0f}s old, max {MAX_STALE_SECONDS}s)")
    ...
    subprocess.Popen(
        [sys.executable, "-u", str(SCOUT_SCRIPT), "start", "--force"],
