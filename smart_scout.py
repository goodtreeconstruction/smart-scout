...
class ScoutService:
    ...
    # ─────────────────────────────────────────────────────────────
    # Slash Commands — /new_chat etc.
    # ─────────────────────────────────────────────────────────────

    def new_chat(self) -> bool:
        """Click the 'New chat' link in Claude's sidebar to start a fresh conversation."""
        win = self._find_claude_window()
        if not win:
            self.last_error = "Claude window not found"
            print("[Scout V2] X new_chat: Claude window not found")
            return False
        ...
        try:
            ...
            # Look for "New chat" hyperlink or button in sidebar
            found = False
            for el in win.descendants():
                try:
                    ...
                    if name == "New chat" and ct in ("Hyperlink", "Button", "Text"):
                        print(f"[Scout V2] Found new chat element ({ct}), clicking...")
                        el.click_input()
                        found = True
                        break
                except Exception:
            ...
            if not found:
                print("[Scout V2] New chat element not found, trying Ctrl+N...")
                pyautogui.hotkey('ctrl', 'n')
                found = True

            time.sleep(1.0)
            print("[Scout V2] New chat started" if found else "[Scout V2] X Could not start new chat")
            return found

        except Exception as e:
            self.last_error = f"new_chat error: {e}"
            print(f"[Scout V2] X new_chat error: {e}")
            return False
        finally:
